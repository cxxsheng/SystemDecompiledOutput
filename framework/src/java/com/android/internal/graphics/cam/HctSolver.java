...
public class HctSolver {
    ...
    static double chromaticAdaptation(double d) {
        ...
        return ((com.android.internal.graphics.cam.CamUtils.signum(d) * 400.0d) * pow) / (pow + 27.13d);
    }

    static double hueOf(double[] dArr) {
        double[][] dArr2 = SCALED_DISCOUNT_FROM_LINRGB;
        double d = (dArr[0] * dArr2[0][0]) + (dArr[1] * dArr2[0][1]) + (dArr[2] * dArr2[0][2]);
        double d2 = (dArr[0] * dArr2[1][0]) + (dArr[1] * dArr2[1][1]) + (dArr[2] * dArr2[1][2]);
        double d3 = (dArr[0] * dArr2[2][0]) + (dArr[1] * dArr2[2][1]) + (dArr[2] * dArr2[2][2]);
        double chromaticAdaptation = chromaticAdaptation(d);
        double chromaticAdaptation2 = chromaticAdaptation(d2);
        double chromaticAdaptation3 = chromaticAdaptation(d3);
    ...
    static double[] nthVertex(double d, int i) {
        double d2 = Y_FROM_LINRGB[0];
        double d3 = Y_FROM_LINRGB[1];
        double d4 = Y_FROM_LINRGB[2];
        double d5 = i % 4 <= 1 ? 0.0d : 100.0d;
        double d6 = i % 2 != 0 ? 100.0d : 0.0d;
        if (i >= 4) {
            if (i < 8) {
                double d7 = ((d - (d2 * d6)) - (d4 * d5)) / d3;
                if (isBounded(d7)) {
        ...
        double d9 = ((d - (d3 * d5)) - (d4 * d6)) / d2;
        if (isBounded(d9)) {
            return new double[]{d9, d5, d6};
        }
        return new double[]{-1.0d, -1.0d, -1.0d};
    }
    ...
    static int bisectToLimit(double d, double d2) {
        ...
        for (int i = 0; i < 3; i++) {
            if (dArr[i] != dArr2[i]) {
                ...
                while (i2 < 8 && java.lang.Math.abs(i3 - criticalPlaneAbove) > 1) {
                    int floor = (int) java.lang.Math.floor((criticalPlaneAbove + i3) / 2.0d);
        ...
        return com.android.internal.graphics.cam.CamUtils.argbFromLinrgbComponents((dArr[0] + dArr2[0]) / 2.0d, (dArr[1] + dArr2[1]) / 2.0d, (dArr[2] + dArr2[2]) / 2.0d);
    }

    static double inverseChromaticAdaptation(double d) {
        double abs = java.lang.Math.abs(d);
        return com.android.internal.graphics.cam.CamUtils.signum(d) * java.lang.Math.pow(java.lang.Math.max(0.0d, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }

    static int findResultByJ(double d, double d2, double d3) {
        double sqrt = java.lang.Math.sqrt(d3) * 11.0d;
        com.android.internal.graphics.cam.Frame frame = com.android.internal.graphics.cam.Frame.DEFAULT;
        double pow = 1.0d / java.lang.Math.pow(1.64d - java.lang.Math.pow(0.29d, frame.getN()), 0.73d);
        double cos = (java.lang.Math.cos(d + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * frame.getNc() * frame.getNcb();
        ...
        while (i < 5) {
            ...
            double d6 = sqrt;
            double aw = (frame.getAw() * java.lang.Math.pow(d4, (1.0d / frame.getC()) / frame.getZ())) / frame.getNbb();
            ...
            double inverseChromaticAdaptation2 = inverseChromaticAdaptation(d12);
            double inverseChromaticAdaptation3 = inverseChromaticAdaptation(d13);
            double[][] dArr = LINRGB_FROM_SCALED_DISCOUNT;
            double d14 = (dArr[0][0] * inverseChromaticAdaptation) + (dArr[0][1] * inverseChromaticAdaptation2) + (dArr[0][2] * inverseChromaticAdaptation3);
            double d15 = (dArr[1][0] * inverseChromaticAdaptation) + (dArr[1][1] * inverseChromaticAdaptation2) + (dArr[1][2] * inverseChromaticAdaptation3);
            double d16 = (inverseChromaticAdaptation * dArr[2][0]) + (inverseChromaticAdaptation2 * dArr[2][1]) + (inverseChromaticAdaptation3 * dArr[2][2]);
            if (d14 < 0.0d || d15 < 0.0d || d16 < 0.0d) {
                return 0;
            }
            double d17 = (Y_FROM_LINRGB[0] * d14) + (Y_FROM_LINRGB[1] * d15) + (Y_FROM_LINRGB[2] * d16);
            if (d17 <= 0.0d) {
                return 0;
            }
            if (i != 4) {
                double d18 = d17 - d3;
                if (java.lang.Math.abs(d18) >= 0.002d) {
                    sqrt = d6 - ((d18 * d6) / (d17 * 2.0d));
                    i++;
                    pow = d5;
                }
            }
            ...
            return com.android.internal.graphics.cam.CamUtils.argbFromLinrgbComponents(d14, d15, d16);
        }
        return 0;
    }
    ...
    public static int solveToInt(double d, double d2, double d3) {
        if (d2 < 1.0E-4d || d3 < 1.0E-4d || d3 > 99.9999d) {
            return com.android.internal.graphics.cam.CamUtils.argbFromLstar(d3);
        }
        double radians = java.lang.Math.toRadians(sanitizeDegreesDouble(d));
        double yFromLstar = com.android.internal.graphics.cam.CamUtils.yFromLstar(d3);
        int findResultByJ = findResultByJ(radians, d2, yFromLstar);
        if (findResultByJ != 0) {
            return findResultByJ;
        }
        return bisectToLimit(yFromLstar, radians);
    }
    ...
    public static com.android.internal.graphics.cam.Cam solveToCam(double d, double d2, double d3) {
        return com.android.internal.graphics.cam.Cam.fromInt(solveToInt(d, d2, d3));
    }
}
