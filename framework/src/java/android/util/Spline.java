...
public abstract class Spline {
    ...
    public static android.util.Spline createSpline(float[] fArr, float[] fArr2) {
        if (!isStrictlyIncreasing(fArr)) {
            throw new java.lang.IllegalArgumentException("The control points must all have strictly increasing X values.");
        }
    ...
    public static android.util.Spline createMonotoneCubicSpline(float[] fArr, float[] fArr2) {
        return new android.util.Spline.MonotoneCubicSpline(fArr, fArr2);
    }

    public static android.util.Spline createLinearSpline(float[] fArr, float[] fArr2) {
        return new android.util.Spline.LinearSpline(fArr, fArr2);
    }

    private static boolean isStrictlyIncreasing(float[] fArr) {
        if (fArr == null || fArr.length < 2) {
            throw new java.lang.IllegalArgumentException("There must be at least two control points.");
        }
        float f = fArr[0];
        int i = 1;
        while (i < fArr.length) {
    ...
    public static class MonotoneCubicSpline extends android.util.Spline {
        ...
        public MonotoneCubicSpline(float[] fArr, float[] fArr2) {
            if (fArr == null || fArr2 == null || fArr.length != fArr2.length || fArr.length < 2) {
                throw new java.lang.IllegalArgumentException("There must be at least two control points and the arrays must be of equal length.");
            }
            ...
            for (int i5 = 0; i5 < i; i5++) {
                if (fArr3[i5] == 0.0f) {
                    ...
                    if (f2 < 0.0f || f3 < 0.0f) {
                        throw new java.lang.IllegalArgumentException("The control points must have monotonic Y values.");
                    }
                    float hypot = (float) java.lang.Math.hypot(f2, f3);
                    if (hypot > 3.0f) {
                        float f4 = 3.0f / hypot;
        ...
        public float interpolate(float f) {
            ...
            while (true) {
                ...
                if (f < this.mX[i3]) {
                    ...
                    float f4 = 2.0f * f3;
                    float f5 = (this.mY[i] * (f4 + 1.0f)) + (this.mM[i] * f2 * f3);
                    float f6 = 1.0f - f3;
        ...
        public java.lang.String toString() {
            ...
            for (int i = 0; i < length; i++) {
                ...
                sb.append(android.inputmethodservice.navigationbar.NavigationBarInflaterView.KEY_CODE_START).append(this.mX[i]);
                sb.append(", ").append(this.mY[i]);
                sb.append(": ").append(this.mM[i]).append(android.inputmethodservice.navigationbar.NavigationBarInflaterView.KEY_CODE_END);
            }
            sb.append("]}");
            return sb.toString();
        }
