...
public class ContentCaptureServiceInfo {
    private static final java.lang.String TAG = android.service.contentcapture.ContentCaptureServiceInfo.class.getSimpleName();
    ...
    public ContentCaptureServiceInfo(android.content.Context context, android.content.ComponentName componentName, boolean z, int i) throws android.content.pm.PackageManager.NameNotFoundException {
        this(context, getServiceInfoOrThrow(componentName, z, i));
    }

    private ContentCaptureServiceInfo(android.content.Context context, android.content.pm.ServiceInfo serviceInfo) {
        android.content.res.TypedArray typedArray;
        if (!android.Manifest.permission.BIND_CONTENT_CAPTURE_SERVICE.equals(serviceInfo.permission)) {
            android.util.Slog.w(TAG, "ContentCaptureService from '" + serviceInfo.packageName + "' does not require permission " + android.Manifest.permission.BIND_CONTENT_CAPTURE_SERVICE);
            throw new java.lang.SecurityException("Service does not require permission android.permission.BIND_CONTENT_CAPTURE_SERVICE");
        }
        this.mServiceInfo = serviceInfo;
        android.content.res.XmlResourceParser loadXmlMetaData = serviceInfo.loadXmlMetaData(context.getPackageManager(), android.service.contentcapture.ContentCaptureService.SERVICE_META_DATA);
        ...
        try {
            ...
            if (XML_TAG_SERVICE.equals(loadXmlMetaData.getName())) {
                try {
                    typedArray = resourcesForApplication.obtainAttributes(android.util.Xml.asAttributeSet(loadXmlMetaData), com.android.internal.R.styleable.ContentCaptureService);
                ...
                android.util.Log.e(TAG, "Meta-data does not start with content-capture-service tag");
            }
