package android.service;

/* loaded from: classes3.dex */
public final class SensorPrivacyIndividualEnabledSensorProto {
    public static final int AUTO_DRIVER_ASSISTANCE_APPS = 5;
    public static final int AUTO_DRIVER_ASSISTANCE_HELPFUL_APPS = 3;
    public static final int AUTO_DRIVER_ASSISTANCE_REQUIRED_APPS = 4;
    public static final int CAMERA = 2;
    public static final int DISABLED = 2;
    public static final int ENABLED = 1;
    public static final int HARDWARE = 2;
    public static final long IS_ENABLED = 1133871366146L;
    public static final long LAST_CHANGE = 1112396529667L;
    public static final int MICROPHONE = 1;
    public static final long SENSOR = 1159641169921L;
    public static final int SOFTWARE = 1;
    public static final long STATE_TYPE = 1159641169925L;
    public static final long TOGGLE_TYPE = 1159641169924L;
    public static final int UNKNOWN = 0;
}
