...
final class ProgramInfoCache {
    ...
    @com.android.internal.annotations.VisibleForTesting
    void updateFromHalProgramListChunk(android.hardware.broadcastradio.ProgramListChunk programListChunk) {
        if (programListChunk.purge) {
            this.mProgramInfoMap.clear();
        }
        for (int i = 0; i < programListChunk.modified.length; i++) {
            android.hardware.radio.RadioManager.ProgramInfo programInfoFromHalProgramInfo = com.android.server.broadcastradio.aidl.ConversionUtils.programInfoFromHalProgramInfo(programListChunk.modified[i]);
            if (programInfoFromHalProgramInfo == null) {
                com.android.server.utils.Slogf.e(TAG, "Program info in program info %s in chunk is not valid", programListChunk.modified[i]);
            } else {
                putInfo(programInfoFromHalProgramInfo);
            }
        }
        if (programListChunk.removed != null) {
            for (int i2 = 0; i2 < programListChunk.removed.length; i2++) {
                this.mProgramInfoMap.remove(com.android.server.broadcastradio.aidl.ConversionUtils.identifierFromHalProgramIdentifier(programListChunk.removed[i2]));
            }
        }
        this.mComplete = programListChunk.complete;
    }

    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndUpdateFromInternal(com.android.server.broadcastradio.aidl.ProgramInfoCache programInfoCache, boolean z) {
        return filterAndUpdateFromInternal(programInfoCache, z, 100, 500);
    }

    @com.android.internal.annotations.VisibleForTesting
    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndUpdateFromInternal(com.android.server.broadcastradio.aidl.ProgramInfoCache programInfoCache, boolean z, int i, int i2) {
        boolean z2;
        if (z) {
            this.mProgramInfoMap.clear();
        }
    ...
    @android.annotation.Nullable
    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndApplyChunk(android.hardware.broadcastradio.ProgramListChunk programListChunk) {
        return filterAndApplyChunkInternal(programListChunk, 100, 500);
    }

    @com.android.internal.annotations.VisibleForTesting
    @android.annotation.Nullable
    java.util.List<android.hardware.radio.ProgramList.Chunk> filterAndApplyChunkInternal(android.hardware.broadcastradio.ProgramListChunk programListChunk, int i, int i2) {
        if (programListChunk.purge) {
            this.mProgramInfoMap.clear();
        }
        android.util.ArraySet arraySet = new android.util.ArraySet();
        for (int i3 = 0; i3 < programListChunk.modified.length; i3++) {
            android.hardware.radio.RadioManager.ProgramInfo programInfoFromHalProgramInfo = com.android.server.broadcastradio.aidl.ConversionUtils.programInfoFromHalProgramInfo(programListChunk.modified[i3]);
            if (programInfoFromHalProgramInfo == null) {
                com.android.server.utils.Slogf.w(TAG, "Program info %s in program list chunk is not valid", programListChunk.modified[i3]);
        ...
        android.util.ArraySet arraySet2 = new android.util.ArraySet();
        if (programListChunk.removed != null) {
            for (int i4 = 0; i4 < programListChunk.removed.length; i4++) {
                android.hardware.radio.ProgramSelector.Identifier identifierFromHalProgramIdentifier = com.android.server.broadcastradio.aidl.ConversionUtils.identifierFromHalProgramIdentifier(programListChunk.removed[i4]);
                if (identifierFromHalProgramIdentifier == null) {
                    com.android.server.utils.Slogf.w(TAG, "Removed identifier %s in program list chunk is not valid", programListChunk.modified[i4]);
                } else if (this.mProgramInfoMap.containsKey(identifierFromHalProgramIdentifier)) {
                    arraySet2.addAll(this.mProgramInfoMap.get(identifierFromHalProgramIdentifier).keySet());
